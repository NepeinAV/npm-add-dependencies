...
class AddDependencies {
  ...
  runNpmShow(dep) {
    const arrayRegex = /\[(.+?)\]/;
    const depMatch = dep.match(arrayRegex);

    if (depMatch) {
      return new Promise((resolve) => {
        const [depName, depUrl] = depMatch[1].split(',');

        this.result[depName] = `${depUrl}`;

        console.log(`Processed: ${depName}, latest version: ${depUrl}`);
    ...
    const [depName, depVersion] = dep.charAt(0) !== '@' 
      ? depSplit
      : [`@${depSplit[1]}`, depSplit[2]];
